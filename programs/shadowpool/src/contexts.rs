...
/// — clients pre-compute them from `lb_pair.active_id`.
///
/// Validation layers (audit-defensible stack):
/// 1. `cranker == vault.cranker` — authority-gated (Phase 0).
/// 2. `lb_pair.owner == dlmm::ID` — Anchor owner check; blocks spoofed
///    pool accounts passed via typosquatting or program confusion.
/// 3. Mint pair equality — the two mints on the DLMM pool must equal
///    the two mints on the vault (in either X/Y ordering). Rejects a
///    cranker pointing at a pool for a different asset pair.
/// 4. DLMM's own internal checks during swap — validates
///    `user_token_in.mint ∈ {pool token_x_mint, pool token_y_mint}`,
///    direction inference from ordering, reserves integrity,
///    slippage vs `min_amount_out`, and bin-array completeness.
/// 5. Slippage floor computed in-handler from MPC-revealed prices
///    (capped at MAX_ALLOWED_SLIPPAGE_BPS) — cranker-supplied
///    `min_amount_out` cannot be looser than this bound.
#[derive(Accounts)]
pub struct ExecuteRebalance<'info> {
    // Cranker gate — see Phase 0 (H-1).
    #[account(
        mut,
        constraint = cranker.key() == vault.cranker @ ErrorCode::Unauthorized,
    )]
    ...
    // --- Meteora DLMM swap accounts (IDL order) ---

    /// DLMM pool state. Owner-checked against the Meteora DLMM program.
    /// CHECK: internal layout validated by DLMM during the swap CPI.
    #[account(mut, owner = dlmm_cpi::dlmm_program::ID @ ErrorCode::InvalidDlmmPool)]
    pub lb_pair: UncheckedAccount<'info>,

    /// DLMM token-X reserve. Writable; DLMM moves tokens in/out during swap.
    /// CHECK: validated by DLMM against lb_pair internal state.
    #[account(mut)]
    ...
    #[account(
        constraint = (
            token_x_mint.key() == vault.token_a_mint
                && token_y_mint.key() == vault.token_b_mint
        ) || (
            token_x_mint.key() == vault.token_b_mint
                && token_y_mint.key() == vault.token_a_mint
    ...
    /// CHECK: required by DLMM's emit_cpi! pattern; not deserialised.
    pub dlmm_event_authority: UncheckedAccount<'info>,

    /// DLMM program self-reference, required by emit_cpi!.
    /// CHECK: address-pinned to the DLMM program id.
    #[account(address = dlmm_cpi::dlmm_program::ID @ ErrorCode::InvalidDlmmPool)]
    pub dlmm_program: UncheckedAccount<'info>,

    // `ctx.remaining_accounts` holds the bin arrays the swap will walk.
    // Typically 3 (active-1, active, active+1) pre-computed client-side
    // from `lb_pair.active_id`.
}
