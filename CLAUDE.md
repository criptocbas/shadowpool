...
### Shipped
- **Program**: 22 instructions (including `close_vault` for winding down stale/legacy vaults and `emergency_override` for the M-2 escape hatch), modular src/ layout, `token_interface` migration done, rustdoc on every public instruction.
...
3. ~~**H-3: Pre/post reload accounting in deposit/withdraw**~~ — ✅ shipped. See gotcha #17. Deposit credits `balance_after - balance_before`.
