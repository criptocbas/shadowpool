...
- **Devnet deployed**: program live at `BEu9VWMdba4NumzJ3NqYtHysPtCWe1gB33SbDwZ64g4g`, last upgrade slot 456227215 (2026-04-17). All 5 comp-defs initialized on cluster 456. IDL account fresh. A stale test vault from pre-Phase-0 still sits at `7L7svML23JXnFGkhKpZjTAoTaNnV6bvy7AdfYvdkQzrP` with the old 468-byte layout — the new IDL expects 549 bytes and can't deserialize it. For tests, either use a different authority keypair (cleanest) or add a `close_vault` instruction so the old one can be wound down (Phase 2 addition). `scripts/check-devnet-state.ts` prints the current on-chain state in one shot.
